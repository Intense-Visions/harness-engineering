...
- **types**: additive `BackendCapabilities`, `ComplexityVerdict`, `RoutingRequest`,
  `RoutingPolicy`, `RoutingError`; optional `capabilities?` on `BackendDef`; optional
  `complexity` / `tierRequired` / `estCostUsd` on `RoutingDecision`. `RoutingValue` is
  **not** widened — tier resolution lives entirely in the AMR layer (backward-compatible).
...
- **orchestrator**: `AdaptiveRouter` wraps `BackendRouter` (which is unchanged), a
  ...
  `AdaptiveRouter` only when a `routing.policy` is present.
