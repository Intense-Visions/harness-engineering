...
sourceHash: "0717cd0d7a5466b2a8cb227f1e17ccc614f67871fc6a25c67f8ab67831fb9875"
...
The `packages/cli/src/commands` module is the command registry for the harness CLI, exposing ~100+ command creators (each following the `createXCommand()` pattern) that return configured `commander.js` Command objects. Split into individual files per command with a central `_registry.ts` that auto-generates the alphabetized command list. Commands span verification/checks (`check-arch`, `check-perf`, `check-docs`), generative craft pipelines (`code-craft`, `test-craft`, `spec-craft`), operational tasks (`install`, `uninstall`, `cleanup`, `sync-main`), analysis/intelligence (`recommend`, `insights`, `rollback`, `skill-regression`), and integration points (`setup-mcp`, `orchestrator`, `setup`). The module bridges the CLI frontend (commander.js) and domain logic in `core`, `orchestrator`, `graph`, and other subsystems.

## Invariants

- Auto-generated registry: _registry.ts is auto-generated by pnpm run generate-barrel-exports; hand-edits are overwritten. Every new command must export exactly one createXCommand() from its file.
- Stateless command factories: Each createXCommand() must return a fresh Command instance every call; no state leaks between runs since the CLI instantiates creators at startup.
- Interface contract synchronization: All exports in the contract must match actual module exports or reference-docs generation fails. New exports must be documented or pre-push blocks.
- Command creator naming: All command creators must be named createXCommand() for consistency with regex-based tooling (docs generators, agent-definition builders).
- Config resolution pattern: Commands needing harness config must call resolveConfig() or loadConfig() early; missing config should fail clearly or degrade gracefully.
- Error handling: All errors must be CLIError with explicit ExitCode; unhandled exceptions exit 1 (generic ERROR code).
- Platform mirrors: Skill/agent-definition commands re-export in symlinks at agents/skills/{cursor,codex,gemini-cli}/<command>/; edit one, all four update. Missing mirrors cause CI flag.
- Subsystem coupling: Heavy imports from @harness-engineering/orchestrator, @harness-engineering/graph, and comprehension subsystems; major API changes ripple to all consuming commands.
- Output formatting: All human output routes through centralized OutputFormatter or logger; direct console.log() breaks scripted modes and telemetry.
- Command file isolation: Each command's logic should stay in its own file (add.ts, check-arch.ts, etc.); shared utilities live in subdirectories or ../shared to prevent circular imports.
