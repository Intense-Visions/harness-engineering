...
8. CML does NOT invoke `AnalysisProvider` for `autoExecute` tiers — `alwaysHuman` tiers run SEL for enrichment but skip complexity scoring.
...
12. `alwaysHuman` tiers get SEL enrichment attached to `EscalateEffect` — routing unchanged, but humans get pre-analyzed context.
13. All existing orchestrator tests pass after integration — zero regressions.
...
1. Create `pipeline.ts` with `IntelligencePipeline` class:
   ...
   - `async preprocessIssue(issue: Issue, scopeTier: ScopeTier): Promise<{ spec: EnrichedSpec | null; score: ComplexityScore | null; signals: ConcernSignal[] }>`
   - `preprocessIssue()` composes: `toRawWorkItem()` → `enrich()` → `score()` → `scoreToConcernSignals()`
   - `autoExecute` → skip everything (null spec, null score, empty signals)
   - `alwaysHuman` → run SEL only, return enriched spec but null score and empty signals
   - `signalGated` → full pipeline (unchanged)
...
3. Write integration test with mock `AnalysisProvider` and mock `GraphStore`:
   ...
   - `alwaysHuman` tier → produces non-null spec with exactly one `AnalysisProvider` call (SEL), but null score and empty signals
...
**Files:** `packages/orchestrator/package.json`, `packages/orchestrator/src/orchestrator.ts`, `packages/orchestrator/src/core/state-machine.ts`, `packages/orchestrator/src/types/events.ts`, `packages/types/src/orchestrator.ts`

1. Add `@harness-engineering/intelligence` to `packages/orchestrator/package.json` dependencies
2. In `packages/types/src/orchestrator.ts`, add `enrichedSpec?: EnrichedSpec` to `EscalateEffect`
3. In `packages/orchestrator/src/types/events.ts`, extend the `tick` event type:
   - Add `concernSignals?: Map<string, ConcernSignal[]>` (issueId → signals)
   - Add `enrichedSpecs?: Map<string, EnrichedSpec>` (issueId → spec)
4. In `packages/orchestrator/src/orchestrator.ts`:
   - If `config.intelligence?.enabled`, instantiate `IntelligencePipeline`
   - In `tick()`: For each candidate, call `detectScopeTier()`.
   - If tier is `signalGated` OR `alwaysHuman`, call `await this.pipeline.preprocessIssue(issue, scopeTier)`.
   - Collect `signals` and `spec` into the tick event maps.
   - `alwaysHuman` now runs SEL for enrichment context, even though routing is pre-determined.
5. In `packages/orchestrator/src/core/state-machine.ts`:
   - Modify tick handler and retry path: `routeIssue(scopeTier, signals, escalationConfig, event.enrichedSpecs?.get(issue.id))`
   - Update `routeIssue()` (or the effect creation logic) to attach `enrichedSpec` to `EscalateEffect` when routing to human.
   - Ensure `handleEscalation()` (or the interaction queue logic) threads the `enrichedSpec` into the final interaction context.
6. Run full existing test suite — assert zero regressions
7. Run `harness validate` if available
8. Commit: `feat(orchestrator): wire intelligence pipeline into dispatch path with enrichment support`
...
| Risk                                                             | Mitigation                                                                                                                                                                           |
| ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| LLM calls in SEL add latency to the tick cycle                   | Pipeline skips SEL/CML for `autoExecute`. `alwaysHuman` runs SEL only (no CML). Only `signalGated` triggers full pipeline. Config toggle allows disabling entirely.                  |
