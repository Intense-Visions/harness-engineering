...
- **`harness roadmap sync` + CI-safety guards** — The full bidirectional roadmap↔tracker sync (`fullSync`) is now reachable from the CLI, not only through the `manage_roadmap action:"sync"` MCP tool. CI could previously only ever flip rows **to `done`** (via `roadmap reconcile`); every other transition and the whole tracker-label push depended on a human remembering, which is how one downstream repo ended up with `last_synced` 22 days behind `last_manual_edit` and 22 issues carrying no tracker labels at all (invisible to a tracker scoped by a selector label). The command is **dry-run by default** — `--apply` is required to write anything. `ExternalSyncOptions` gains three options, all defaulting to today's behaviour: `dryRun` (compute and report every intended change, perform zero write requests — no ticket create, no ticket patch, no roadmap writeback), `allowCreate` (default `true`; when `false`, `syncToExternal` reports each skipped row in `skippedCreates` instead of creating a ticket), and `syncIssueState` (default `true`; when `false`, the issue `state` field is omitted from every patch body, so labels converge but no issue is ever closed or reopened — reported in `skippedStateChanges`). Both write paths honour the state guard: `GitHubIssuesSyncAdapter.updateTicket` (new optional `TicketWriteOptions` 4th argument) and `GitHubIssuesTrackerAdapter.buildIssuePatchBody` (new `syncIssueState` constructor option); the careful label-preservation logic — skip the labels field entirely when the refresh GET fails, so a transient blip cannot wipe the `harness-managed` selector — is unchanged on both settings. `SyncResult` gains `dryRun`, `planned`, `skippedCreates`, `skippedStateChanges`, and `examined` (the denominator: rows compared and tickets fetched, `null` when the fetch failed). Denominator discipline is enforced at the CLI edge: the new `ExitCode.ZERO_DENOMINATOR` (3) fires when zero roadmap rows were parsed or zero tickets were fetched with a tracker configured — a sync that matched nothing has abstained, not succeeded, and must never read as a pass. Intended consumer pattern: a nightly `harness roadmap sync --apply --no-create --no-state-change` converges labels safely while issue closure stays with the PR-merge auto-done path. See [`docs/guides/roadmap-sync.md`](docs/guides/roadmap-sync.md) §"Running Sync from the CLI". (`@harness-engineering/cli`, `@harness-engineering/core`, `@harness-engineering/types`)
- **Granular Task→Backend Routing (Spec B)** — Per-skill and per-cognitive-mode routing axes; fallback chains as a shared primitive; in-memory decision telemetry with dashboard + CLI surfaces. `RoutingUseCase` (in `packages/types/src/orchestrator.ts`) gains two variants — `{ kind: 'skill'; skillName; cognitiveMode? }` and `{ kind: 'mode'; cognitiveMode }`. `RoutingConfig` gains `skills?: Record<string, RoutingValue>` and `modes?: Record<string, RoutingValue>` maps; **`RoutingValue = string | readonly [string, ...string[]]`** widens every existing scalar routing field (`default`, `quick-fix`, `guided-change`, `full-exploration`, `diagnostic`, `intelligence.{sel,pesl}`, `isolation.<tier>`) to accept ordered fallback chains. **The widening is additive and scalar-compatible — pre-Spec-B configs continue to behave byte-for-byte identically (D4).** `BackendRouter.resolve()` (`packages/orchestrator/src/agent/backend-router.ts`) is rewritten to walk a deterministic chain of sources (invocation override → per-skill → per-cognitive-mode → per-tier/per-intelligence-layer/per-isolation → `routing.default`); first match wins, chain entries within a source are tried in declared order, `unknown-backend` outcomes recorded in `RoutingDecision.resolutionPath` for telemetry. New `--backend <name>` flag on `harness skill run` and `harness dispatch` provides an invocation-level escape hatch (D7). New `RoutingDecisionBus` (`packages/orchestrator/src/routing/decision-bus.ts`) emits every `resolve()` to a per-orchestrator-process in-memory ring buffer (default capacity 500, configurable), broadcasts on the WebSocket topic `routing:decision`, and emits a structured `routing-decision` log line per decision — synchronous-but-non-throwing, subscriber errors isolated (S6). Three new HTTP routes (`packages/orchestrator/src/server/routes/v1/routing.ts`): `GET /api/v1/routing/config` returns current config + resolved chains; `GET /api/v1/routing/decisions?skill=&mode=&backend=&limit=` reads filtered records from the ring buffer; `POST /api/v1/routing/trace` runs `resolve()` without dispatching. Three new CLI commands: `harness routing config|trace|decisions` (`packages/cli/src/commands/routing/`) for shell-accessible inspection. New dashboard route `/routing` (`packages/dashboard/src/client/pages/Routing.tsx`) renders four cards — Resolved Chains, Recent Decisions (filterable, expandable rows), Per-Backend Volume (24 h aggregation), Trace Tool — subscribed to `routing:decision` WS with 5 s HTTP polling fallback when WS is disconnected (O2). Startup config validation (`packages/orchestrator/src/workflow/schema.ts` + `packages/orchestrator/src/workflow/config.ts`) hard-errors on any chain entry that references a backend missing from `agent.backends` (every routing surface — `default`, tier, intelligence, isolation, skills, modes — uniformly checked) and warns on unknown skill names + non-standard cognitive modes (D10). New ADRs codify the architectural choices: [0029 — per-skill and per-cognitive-mode routing axes](docs/knowledge/decisions/0029-per-skill-and-per-mode-routing-axes.md) (D1), [0030 — routing resolution order](docs/knowledge/decisions/0030-routing-resolution-order.md) (D2), [0031 — fallback chains as a shared routing primitive](docs/knowledge/decisions/0031-fallback-chains-shared-routing-primitive.md) (D4), [0032 — routing-decision telemetry via in-memory ring buffer](docs/knowledge/decisions/0032-routing-decision-telemetry-ring-buffer.md) (D8), [0033 — skill authors do not declare backend preferences](docs/knowledge/decisions/0033-skill-authors-do-not-declare-backend-preferences.md) (D6). New knowledge doc [`routing-resolution.md`](docs/knowledge/orchestrator/routing-resolution.md); existing [`issue-routing.md`](docs/knowledge/orchestrator/issue-routing.md) extended with the new axes + resolution order + fallback semantics. New operator guide [`routing-trace.md`](docs/guides/routing-trace.md); existing [`multi-backend-routing.md`](docs/guides/multi-backend-routing.md) extended with per-skill/per-mode section + worked patterns. Composes cleanly with LMLM (Spec A) — routing entries reference backend names whose models LMLM auto-populates; Spec B carries no LMLM-specific code. Spec at [`docs/changes/granular-task-routing/proposal.md`](docs/changes/granular-task-routing/proposal.md). (`@harness-engineering/orchestrator`, `@harness-engineering/types`, `@harness-engineering/cli`, `@harness-engineering/dashboard`)
- **Hermes Phase 4 — Skill Proposal / Refinement Loop** — Agents can emit candidate skills into a review queue via the new `emit_skill_proposal` MCP tool (tier `standard`); proposals land at `.harness/proposals/<id>.json` and are gated by a soundness review before promotion. New schemas in `packages/types/src/proposals.ts` (`SkillProposalSchema`, `EmitSkillProposalInputSchema`, `EditProposalInputSchema`, `SkillProvenanceSchema`, `ProposalKindSchema`, `ProposalStatusSchema`); strict `kind` ↔ content invariants enforce `new-skill` ⇒ `skillYaml + skillMd` and `refinement` ⇒ `targetSkill + diff`. Storage layer in `@harness-engineering/core` (`packages/core/src/proposals/{store,usage,index}.ts`) provides `createProposal`, `getProposal`, `listProposals`, `updateProposal`, and `deriveSkillUsage` (read-only aggregation over existing `skill_invocation` telemetry — no new emission). Orchestrator-side: `packages/orchestrator/src/proposals/gate.ts` runs mechanical structural checks (kebab-case name, parseable skill.yaml, SKILL.md size bounds, unified-diff well-formedness) and persists findings; `promote.ts` writes `agents/skills/claude-code/<name>/{skill.yaml,SKILL.md}` with `provenance: agent-proposed` and `originatingProposalId: <id>` after gate passes, or stamps provenance on refined existing skills; `events.ts` emits `proposal.created`, `proposal.approved`, `proposal.rejected` through the Phase 0 webhook bus and Phase 3 sink registry with envelope derivers in `notifications/envelope.ts`. Seven gateway routes under `/api/v1/proposals/*` (`server/routes/v1/proposals.ts`) — list / get / run-gate / approve / reject / edit; reads use the existing `read-status` scope while mutations require the new `manage-proposals` scope (added to `TokenScopeSchema` and `SCOPE_VOCABULARY`). CLI: `harness proposals list|show|approve|reject` for queue management plus a one-shot `harness backfill-skill-provenance` migration that stamps `provenance: user-authored` onto every existing catalog skill (idempotent). Dashboard page `/s/proposals` (`packages/dashboard/src/client/pages/Proposals.tsx`) renders the queue with inline content, gate findings panel, run-gate / approve / reject / edit actions; reviewer-UX budget is < 30s per proposal per the spec (D9). New maintenance task `proposal-provenance-backfill` (housekeeping, scheduled at Feb 31 so the cron loop never fires it; operators trigger manually). New ADRs [0016 — Skill proposal / refinement workflow](docs/knowledge/decisions/0016-hermes-phase-4-skill-proposal-workflow.md) and [0017 — `manage-proposals` token scope](docs/knowledge/decisions/0017-manage-proposals-scope.md); knowledge nodes [`docs/knowledge/cli/skill-proposals.md`](docs/knowledge/cli/skill-proposals.md) and [`docs/knowledge/cli/skill-provenance.md`](docs/knowledge/cli/skill-provenance.md). Spec at `docs/changes/hermes-phase-4-skill-proposals/proposal.md`. Test coverage: types schema (15 cases), core store + usage derivation (9), CLI MCP tool (8), CLI proposals subcommand (6), orchestrator gate (6), orchestrator promote (7), envelope derivers (4 new rows), event emitters (4), route handlers (10), backfill (6) — 75 new tests across five packages; existing scopes test passes with the new vocabulary entry. (`@harness-engineering/orchestrator`, `@harness-engineering/core`, `@harness-engineering/types`, `@harness-engineering/cli`, `@harness-engineering/dashboard`)
- **Hermes Phase 2 — Custom Maintenance Jobs + Pre-launch OSV Guard + Disk Hygiene** — Extends `MaintenanceScheduler` beyond the 21 built-in tasks with operator-defined `customTasks` in `harness.orchestrator.md`. Custom tasks honor the existing 4-task-type taxonomy (`mechanical-ai` / `pure-ai` / `report-only` / `housekeeping`) — no new types — and gain five new optional fields: `checkScript` (arbitrary executable replacing the legacy `checkCommand: string[]`, with last-line JSON status envelope `{status, findings?, wakeAgent?, message?, outputs?}` parsing and a heuristic fallback), `contextFrom: string[]` (upstream task IDs whose latest persisted output is injected as `## Upstream context` in the agent prompt), `inlineSkills: string[]` + `inlineSkillsBudgetTokens` (skill markdown bodies inlined under `## Reference skills`, warns-then-truncates skill-granularly on overflow at ~4 chars/token), `outputRetention: { runs?, maxAgeDays? }` (overrides the global last-50 / 30-day default). `RunResult.origin: RunOrigin` becomes a discriminated provenance tag (`'cron' | 'cli' | { kind: 'api', tokenName } | { kind: 'chain', upstreamTaskId }`) set by the entry point and never configurable — older dashboards rendering newer payloads fall back to `'—'` because the field is optional on the wire. `TaskOutputStore` (`packages/orchestrator/src/maintenance/output-store.ts`) persists one JSON file per run at `.harness/maintenance/<task-id>/outputs/<iso>.json` with atomic write-and-rename and after-write retention sweeps. `CheckScriptRunner` (`check-script-runner.ts`) spawns via `execFile` (no shell — args pass verbatim), captures stdout + stderr, and resolves the structured envelope vs the legacy regex-extraction. `ContextResolver` (`context-resolver.ts`) reads upstream outputs via the store and inline-skill bodies via an injected `InlineSkillReader`. `validateCustomTasks` (`custom-task-validator.ts`) runs at orchestrator boot: kebab-case ID check, built-in collision check, per-type required fields, mutually-exclusive `checkCommand` vs `checkScript`, DFS cycle detection across the merged `contextFrom` graph (built-ins are leaf nodes), skill / script existence via injected predicates. The built-in 21 tasks continue to run through the legacy `CheckCommandRunner` + `CommandExecutor` paths byte-for-byte unchanged — all 225 existing maintenance tests pass without modification. New `harness mcp-guard check [--strict] [--json]` and `harness mcp-guard cache clear` CLI subcommands wrap a new `createOsvClient` in `@harness-engineering/core` (`packages/core/src/security/osv-client.ts`) that queries `https://api.osv.dev/v1/query`, classifies advisories by ID prefix (`MAL-*` → blocking, else surfaced), caches results at `.harness/cache/osv/<ecosystem>-<name>@<version>.json` with a 24h TTL, and fails open by default (warn-and-continue on network errors) or closed under `--strict`. The guard reads `.mcp.json`, extracts each `npx`-launched `<pkg>[@<version>]` argument, exits 2 on any malicious match — suitable as a `pre-mcp-launch` hook from host plugin manifests. `harness cleanup-sessions` gains `--all` / `--include <list>` / `--exclude <list>` flags; default no-flag behavior is unchanged. Registered targets under `.harness/`: `sessions` (24h TTL), `cache` (7d), `maintenance` (30d), `dashboard-state` (14d), `snapshots` (14d), `analyzer-output` (7d). Per-target TTLs are overridable via the new `cleanup.ttlHours: Record<string, number>` config section. New `harness maintenance list` and `harness maintenance show <task-id> --limit N` CLI subcommands give read-only visibility into the resolved task list and persisted run outputs without booting an orchestrator instance. The `harness maintenance run <task-id>` CLI subcommand and the `POST /api/v1/jobs/maintenance/{taskId}/trigger` + `GET /api/v1/jobs/maintenance/{taskId}/outputs(/{runId})` API routes are deferred to a follow-up that lands alongside the Phase 0 Gateway API contracts; the CLI list/show and the cron + chain trigger paths cover the v1 use cases. New ADR [0015 — Custom maintenance task model](docs/knowledge/decisions/0015-hermes-phase-2-custom-maintenance-jobs.md) and knowledge docs [`custom-maintenance-jobs.md`](docs/knowledge/orchestrator/custom-maintenance-jobs.md) and [`pre-launch-osv-guard.md`](docs/knowledge/cli/pre-launch-osv-guard.md). Spec at `docs/changes/hermes-phase-2-custom-jobs/proposal.md`. Test coverage: 16 validator cases (valid configs, all rejection paths, cycles of length 1/2/N, fork-and-merge graphs, missing skill / script, multiple errors in one pass), 9 output-store cases (write + read, retention by count + age, per-task override, corrupt entry skip, atomic write), 9 context-resolver cases (missing / stale upstream markers, char-budget exhaustion, skipped missing skills), 8 check-script-envelope parser cases (each `status` value, malformed JSON, scan-past-warnings, empty input), 8 OSV client cases (MAL classification, cache hit/miss, fail-open vs fail-closed, HTTP non-2xx, scoped-name sanitization), 12 mcp-guard cases (spec parser, package extraction, malicious vs clean .mcp.json), and 5 cleanup-all cases (per-target TTL, include / exclude / ttlHours overrides, dry-run vs delete). (`@harness-engineering/orchestrator`, `@harness-engineering/core`, `@harness-engineering/types`, `@harness-engineering/cli`)
