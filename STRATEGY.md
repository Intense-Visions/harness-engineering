...
- **Holiday Confidence:** the % of merged PRs over a 30-day rolling window that cleared all four "would-you-leave-this-unwatched" gates — (a) a multi-persona review fired, (b) the post-merge outcome-eval did not fail, (c) no baseline was silently auto-updated during the window, and (d) no curated Signal was in breach. Gates (a) and (b) are evaluated per-PR (a graded pass fraction); (c) and (d) are window-wide conditions that gate the whole window to 0 when breached, because a window where drift crept in or a signal went red was not safe to leave. It is computed from the existing curated-Signal authorities (the `gh` merged-PR list + `## Assessment:` review marker for (a), graph `execution_outcome` nodes for (b), and the `baseline-auto-update-count` / all-Signal statuses for (c)/(d)) by `computeHolidayConfidence` in `@harness-engineering/signals`, and surfaced by `harness holiday-confidence` (`--window`, `--json`). Where the other five KPIs measure inputs, this one operationalizes the whole thesis as a single number: _if the senior disappears for two weeks, what holds?_
