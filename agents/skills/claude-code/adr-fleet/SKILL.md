...
1. **One worktree-isolated subagent per confirmed decision.** Each subagent is briefed to run the **real** per-item decide pipeline for its one decision: `harness-architecture-advisor` in autonomous mode (ANALYZE the codebase → PROPOSE options with trade-offs → DOCUMENT the chosen option as an ADR). It does not hand-write the ADR to skip the advisor, and it does not accept the decision — the drafted ADR the advisor leaves behind is what VERIFY checks for. Feed the decision's answered forks from CONFIRM and its pre-allocated ADR number into the brief.

2. **Draft to the canonical decisions directory at `status: proposed`.** The subagent writes a real ADR file to `docs/knowledge/decisions/NNNN-<slug>.md` (its pre-allocated number) with the repo's required frontmatter and Context/Decision/Consequences sections, carrying `status: proposed`. `proposed` is the explicit never-auto-accept marker that distinguishes a fleet draft from an accepted decision; drafting real files in the canonical location makes the batch reviewable as ordinary ADR diffs.
...
3. **Require all-OS CI green.** Confirm the pushed branch's CI is green on **all three operating systems** plus the enforce and harness checks (`gh pr checks` / `gh run list`). Green on one OS is not green. A subset-red branch is not sign-off-ready — it is reported as failed, and the batch continues.
...
| "CI is green on Linux, this draft is ready"                                                | Green on one OS is not green. Sign-off-ready requires all three operating systems plus the enforce and harness checks. A subset-red branch is reported failed, not carried.              |
