...
| "CI is green on Linux, ship the fix"                                              | Green on one OS is not green. Merge-ready requires all operating systems plus the project's required checks. A subset-red branch is reported failed, not shipped.               |
