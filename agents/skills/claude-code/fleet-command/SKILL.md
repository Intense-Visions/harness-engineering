...
5. **Everything the budget shed, named with its reason** — unscheduled empty-queue fleets, fleets shed by the structural cap policy with the depth that ordered the shed, fleets shed because the **cap was undecidable** and the human trimmed them instead, lanes shed because a serialization deferral would have reached the first non-admitting wave, waves never started because the wall-clock ran out, fleets the human trimmed, and members missing or erroring on their probe. A run that quietly did less than it appeared to is worse than one that did less and said so.

   **A depth-ordered shed is reported with the depth that ordered it, and a shed with no depth behind it is never reported as one.** The two are different facts and a reader acts on them differently: a depth-ordered shed says the policy ran and ranked this lane last, while an undecidable-cap shed says the policy could not run and a person chose. Collapsing them under "shed by the cap" would let a run present a hand-made reduction as a structural one — the report equivalent of a green result with no denominator behind it.
...
- **A cap that cannot be decided is reported as undecidable, never as applied.** When no sweep in contention carries a probed depth, the depth-ordered shed does not run: the over-cap reduction is carried to CONFIRM as the human's trim and every resulting shed is reported as **cap undecidable — reduced by human trim**. No fallback ordering is invented to make the cap look structural, and a partially-probed run sheds by depth only among the known-depth sweeps, reporting that group separately from the trimmed remainder.
...
- **Never report a depth-ordered shed that no depth ordered.** When the sweeps in contention are all unknown-depth the cap is **undecidable**, not applied: carry the reduction to CONFIRM as the human's trim and name it as such. Inventing a fallback ordering to keep the cap looking structural, or filing a hand-made reduction under "over the cap," asserts a mechanism that did not run — the same failure as a verified within-allocation check with no artifact behind it.
