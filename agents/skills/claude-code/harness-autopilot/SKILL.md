...
> **Forward-wired — these are harness-OPTIMISTIC defaults, not guaranteed-installed skills.** A detector that is not installed is **skipped, never a hard halt** — the opposite of a user's typo'd hook — and each **auto-activates** once canary ships it and it reports as installed. Canary ships all four as of **7.2.0**. Availability is matched **prefix-insensitively**, so a catalog reporting the plugin-qualified `canary:canary-cassandra` wires the same detector as the bare name. **Report the denominator and treat a surprising `0/4` as a finding, not as the expected state** — an earlier version of this note asserted that canary shipped none of the four, which made a silent zero read as correct behaviour long after it had stopped being true.
