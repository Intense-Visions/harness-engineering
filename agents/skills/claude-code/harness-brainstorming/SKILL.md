...
0a. **Read STRATEGY.md if present at repo root (upstream grounding).** Strategy is the durable product anchor that ranks above feature-level context — read it first so the rest of EXPLORE can cross-reference. Use a Node one-liner that calls `validateStrategy` from `@harness-engineering/core`, then (when valid) `parseStrategyDoc` + `asStrategyDoc`:

    ```bash
    node -e "import('@harness-engineering/core').then(async m => {
      const v = await m.validateStrategy(process.cwd());
      if (!v.present) { console.log(JSON.stringify({ grounded: false, reason: 'absent' })); return; }
      if (!v.valid) { console.log(JSON.stringify({ grounded: false, reason: 'invalid', error: v.error })); return; }
