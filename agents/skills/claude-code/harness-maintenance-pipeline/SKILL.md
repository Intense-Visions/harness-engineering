...
4. The human replies `dead-code,doc-drift`. Run `harness maintenance run --only dead-code,doc-drift --fix`. The CLI prints to stderr: `--fix: AI fix-agent dispatch is not yet wired (executor dispatcher is a stub repo-wide); checks ran, no PRs were opened.` Relay that verbatim and report `fixed: 0`, `prUrl: null` — do not claim a PR was opened.
