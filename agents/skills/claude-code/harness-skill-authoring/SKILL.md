...
### Phase 1C: DECLARE CAPABILITY ROLES — Service Definition / Provider / Consumer

Every new MCP tool or skill capability is an **extension point** — a seam where behavior can be swapped. A seam is only real when all three roles are filled. Borrowed from the capability-seam model: a capability with only one role filled is not actually swappable — it is accidental single-implementation lock-in dressed up as an extension point.

Before writing any code or `skill.yaml`, name all three roles for the capability you are adding:

1. **Service Definition — what it DEFINES.** The contract: the interface, tool schema, or skill boundary that names the behavior independently of any one implementation. For an MCP tool this is its `inputSchema` and documented output shape; for a skill it is the `## Process` contract and the `skill.yaml` interface. If you cannot state the contract without pointing at a single concrete implementation, the seam is not real yet.

2. **Provider(s) — who PROVIDES it.** At least one concrete implementation that satisfies the Service Definition. Name it. If the honest answer is "only this one, forever," say so explicitly and justify why an extension point is warranted rather than an inline call — otherwise collapse it back to a plain function.

3. **Consumer(s) — who CONSUMES it.** At least one caller that depends on the Service Definition rather than on a specific Provider. Name it. A capability that nothing consumes is dead on arrival; a capability consumed only by its own Provider is not a seam.

**The single-role red flag:** if only one of the three roles is filled, STOP. A Service Definition with no Provider is a promise nobody keeps. A Provider with no Consumer is dead code. A Consumer reaching past the Service Definition into a concrete Provider is lock-in. Name the missing role or drop the extension point and write a direct call instead.

Record the three roles in the skill's `## When to Use` or `## Harness Integration` prose (or the MCP tool's description) so a future maintainer can see the seam without reverse-engineering it.
...
3. **Write `## Process`.** This is the core of the skill. Guidelines for writing good process sections:
   ...
   - **Reference the context-budget trip wire for long-running turns.** If the skill describes a **long-running turn** (a phase agent or fleet lane that grinds through many tool calls in one turn), reference the trip-wire discipline and cite `evaluateContextBudget(usedTokens, window)` from `@harness-engineering/core`. The turn should classify its TOTAL resident tokens as `ok | warn | trip` against window-keyed anchors (`1m` 250K/350K, `200k` 80K/100K, `local` ~30%/~37.5%): on `warn` **converge and flush state to disk**; on `trip` **checkpoint-and-restart** into a cold subagent seeded with a distilled (summarized, not raw-truncated) handoff. This is the intra-turn complement to autopilot's between-phase cold dispatch — see harness-autopilot's "Context-Budget Trip Wire" section.
...
| "There's only one implementation, so I don't need to name a Provider and Consumer" | One implementation is exactly when lock-in hides. Name the Service Definition, the Provider, and the Consumer (Phase 1C). If all three collapse to the same code, you don't have a seam — write a direct call instead of a fake extension point. |
...
- **No half-wired capabilities.** A new MCP tool or skill capability must name all three capability roles (Service Definition, at least one Provider, at least one Consumer) per Phase 1C. A capability with only one role filled is accidental single-implementation lock-in — either name the missing role or collapse the extension point into a direct call.
