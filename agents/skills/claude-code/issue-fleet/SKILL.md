...
2. **Triage each issue on the four axes** from its own signals (title, body, existing labels, linked refs):
   ...
   - **route** — assign the downstream fleet that should own the issue: a decision-shaped issue → `adr-fleet`; a build-shaped feature/enhancement → `roadmap-fleet`; a CI-red / flaky-run issue → `cicd-fleet`; a coverage gap → `test-fleet`; an entropy/hotspot issue → `cleanup-fleet`; a doc-drift / stale-documentation issue → `docs-fleet`; a vulnerability or supply-chain issue → `security-fleet`; a measured perf-budget violation or runtime regression → `perf-fleet`; a code-quality / craft issue → `craft-fleet`. An issue that plausibly fits two stages is a **fork**, not a guess. The routable set is exactly the family roster minus the declared **§Non-destinations** — if a member is neither, that is a gap in this skill, not a licence to guess.
...
5. **Build the `IssueCandidate` record** for each issue:
   ...
   IssueCandidate {
     ...
     route,             // downstream fleet: adr | roadmap | pr | cicd | test | cleanup | docs | security | perf | craft
...
2. **Close only the authorized + verified duplicates.** Close each issue that is both human-authorized (CONFIRM) and independently confirmed a genuine duplicate (VERIFY), with a comment **citing the canonical issue/PR**. Close nothing else — never a bare close, never a re-triage-from-scratch. As each issue reaches its terminal outcome here (mutations applied, or closed as a duplicate), **release its cross-run claim** — remove the `fleet:claimed` label, leaving the claim comment as an audit trail (see §Cross-run claim lease); a parked/failed issue releases the label too, so nothing is stranded.
...
Three `-fleet` members are deliberately **not** routable destinations. They are listed here rather than omitted, because an unexplained absence is indistinguishable from an oversight — which is exactly how four real fleets sat unroutable while the enum looked complete:

- **`ideate-fleet`** — it sits _upstream_ of intake and its queue is strategy themes, not issues. An existing issue has already been conceived; routing it back to ideation would run the conveyor backwards.
- **`issue-fleet`** — itself. Routing an issue to intake is a no-op that would loop the queue.
- **`bug-fleet`** — its queue is **derived from the standing codebase**, not from the tracker: it hunts _undiscovered_ defects and ranks code areas by latent risk. An issue is by definition already discovered, so it belongs with the member that fixes its shape (`roadmap-fleet` for a build-shaped fix, `cicd-fleet` for a CI-red one), not with the member that finds new ones.

The route enum and this list must together account for **every** member of the family roster in `docs/reference/fleet-family.md`. A new member belongs on one side or the other before it ships; `agents/skills/tests/issue-fleet-routes.test.ts` fails until it does.
...
- **The downstream fleets** — `adr-fleet`, `roadmap-fleet`, `pr-fleet`, `cicd-fleet`, `test-fleet`, `cleanup-fleet`, `docs-fleet`, `security-fleet`, `perf-fleet`, `craft-fleet` — consume the routed queue issue-fleet emits; issue-fleet routes to them and never does their work.
