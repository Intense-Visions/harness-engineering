...
2. **Apply the evidence gate — a hard filter, not a confidence score.** Assign each candidate exactly one of four named evidence classes, or discard it:
   ...
   - **`advisory-match`** — a resolved dependency version falls inside an advisory's affected range. The range match **is** the class: a resolved version inside a published affected range is a concrete fact, not a guess. Reachability is a **strength modifier, not a second gate** — record `evidenceStrength` as `reached` (the vulnerable API surface is actually called from this codebase) or `lockfile-only` (present in the resolved tree, no call path found). Strength feeds the score and the tier; a `lockfile-only` match ranks below a `reached` one and is **never inflated** to look reached.
   - **`exposed-credential`** — a live-shaped secret literal is present in tracked source, evidenced by **file, line, and credential type**. The value itself is never recorded, quoted, or transported (see the secret rule in DISPATCH). A placeholder, an obvious test fixture, or a documented example value is not a live-shaped secret.
...
6. **Build the `SecurityFinding` record** for each survivor, and detect known decision forks without answering them:
   ...
   SecurityFinding {
     ...
     evidenceClass,    // "reachable-sink" | "exploitable-path" | "advisory-match" | "exposed-credential"
     evidence,         // the concrete trace: sink path, boundary crossing, advisory id + range, or file/line/credential-type
     evidenceStrength, // "reached" | "lockfile-only" — advisory-match only; feeds score and tier
     severity,
...
1. **FIX tier: one worktree-isolated subagent per finding**, briefed to run four steps in order.
   ...
   (d) Run the OWASP/CWE reviewer (`harness-security-review`) **over its own diff** before pushing, so obvious new weaknesses are caught before the branch is pushed. Self-review is a first pass, not the verdict — VERIFY re-scans independently precisely because a subagent reviewing its own work is the weakest link in the chain.
...
7. **Classify each item with a tier-appropriate verdict.** A FIX-tier item is `verified` (all applicable checks passed), `rejected` (missing artifact, evidence still reproduces, a new finding introduced, or definitively red CI), or `retry` (transient, retried **at most once**). A FILE-tier item is `filed-and-verified` (the issue exists, carries its evidence class and trace, and is not a duplicate) or `rejected` (missing, evidence-free, or a duplicate). An item that never reached VERIFY — parked on an unforeseen fork, re-tiered mid-flight, or dropped as a false positive — is recorded as `parked` with its reason. No item reaches REPORT as merge-ready without passing every check that applies to its tier.
...
| "The advisory is rated HIGH, so this is a HIGH finding"                   | Severity describes the weakness in the abstract; evidence strength and exposure describe what it means _here_. A `lockfile-only` match on code this project never calls is a weaker finding and must be labeled as one, never inflated to look reached. |
...
Phase 1: SELECT
  ...
  Evidence gate (hard filter):
    11 carry a named evidence class -> reachable-sink 4, exploitable-path 3, advisory-match 3, exposed-credential 1
    22 discarded (no class: "looks dangerous", "consider hardening this", style-only nits)
  ...
  Scored by severity x evidence strength x exposure:
    "legacy-yaml" is HIGH but lockfile-only on a dev-only dep -> ranked BELOW a MODERATE
    reachable sink at a public entry point. Labeled lockfile-only, never inflated to reached.
  Detected forks: 1 — "stream-parse: the fixed version is outside the compatible range; accept a major bump, or file as structural?"
...
Phase 2: CONFIRM  [checkpoint:human-verify]
  ...
  Human answers fork: stream-parse -> accept the major bump as a bounded fix.
  Human RE-TIERS "password-reset-flow" FIX -> FILE (the patch looked riskier than it read).
...
Phase 3: DISPATCH (governor = 2)
  ...
  "tenant-boundary" proves STRUCTURAL mid-flight (the sink cannot close without moving a
...
Phase 4: VERIFY (independent — never self-report)
  ...
  config-loader    exposed-credential artifact ok, credential gone from source, rotation flagged  -> verified
...
Phase 5: REPORT
  ...
  | stream-parse        | advisory-match     | fix  | verified           | link       | accepted a major bump  | —                    |
  | config-loader       | exposed-credential | fix  | verified           | link       | rotation left to human | —                    |
  | upload-path         | exploitable-path   | fix  | rejected           | —          | —                      | new finding in fix   |
  | tenant-boundary     | reachable-sink     | file | filed-and-verified | link       | —                      | re-tiered mid-flight |
  | password-reset-flow | exploitable-path   | file | filed-and-verified | link       | —                      | re-tiered at CONFIRM |
  | admin-role-model    | exploitable-path   | file | filed-and-verified | link       | —                      | —                    |
  | legacy-yaml         | advisory-match     | file | filed-and-verified | link       | labeled lockfile-only  | —                    |
  | token-signing       | reachable-sink     | file | filed-and-verified | link       | —                      | —                    |
  ...
  Never merged. 3 fix PRs + 5 filed evidence packets handed over for review (1 fix rejected).
```
...
### Scenario 5: Self-report — accepting "fixed, scanner clean, CI green" without checking

A FIX-tier subagent returns a branch and reports that the finding is fixed, the scanner is clean, and CI is green. Expected: the **"a self-report is never verification"** half of the Iron Law requires the orchestrator to independently confirm all four checks — the `docs/changes/<slug>/plans/` artifact and autopilot-state exist, the original evidence no longer reproduces, a re-scan shows no new finding, and CI is green on **all** operating systems — before the item is marked merge-ready. Marking it verified on the subagent's summary, however confident, is the failure this scenario guards against.
