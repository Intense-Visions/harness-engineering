...
1. **WRITE.** Assemble the same section prompts Phase 1 would ask (Target problem, Our approach, Who it's for, Key metrics, Tracks, and the optional sections when the human opts in), in template order, into `docs/interviews/strategy-questions.md` (or `.harness/sessions/<slug>/interviews/strategy-questions.md` when a session slug is known). Each prompt gets an empty `[Answer]:`. Report the path and stop.
