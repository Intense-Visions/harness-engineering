...
| -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "I removed the defect, so I will set `fixed: true` in the recovery record."                              | `fixed` credits only when the planted defect is genuinely resolved, VERIFIED by re-running the fixture's expected check against the scratch copy. Assuming success inflates the record and defeats the whole point of rehearsing on a rope.   |
| "The planted defect is right there in the manifest, so I will read `plantedFile` to guide the recovery." | Reading the answer key (`plantedFile` / `plantedDescription`) into the recovering agent's context makes the rehearsal dishonest — detection credit must come from the agent actually noticing the defect, not from being handed the location. |
| "The recovery obviously passes, so I will just report 100/100."                                          | The score is computed in TypeScript by `harness rehearse score` from the manifest plus the structured record — never asserted by the agent. Hand-waving a number bypasses the referee and makes the result unreproducible.                    |
| "It is faster to fix the defect directly in the fixture directory."                                      | The recovering agent must work only in an isolated scratch copy. Modifying the shipped fixture or the surrounding repo corrupts the ground truth for every future rehearsal and for the gate-regression signal the fixtures provide.          |
| "This fixture scores lower than it used to, so I will tweak the fixture or loosen the rubric."           | A lower score for the SAME recovery quality signals a regression in the harness gate the fixture exercises — investigate the check, not the fixture. Loosening the fixture hides exactly the regression it was built to catch.                |
...
### Example: fixed by luck, wrong gate, and a broken sibling (partial)

The agent removes the secret but never runs a security check, and the edit breaks an unrelated sibling file. `detected` and `fixed` credit; `correctCheck` and `noCollateral` both slip -> 30 + 35 = **65/100, partial**. (Had `noCollateral` held, the score would sit at exactly 80 — a boundary **pass**, not a partial.)
