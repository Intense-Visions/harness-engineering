...
- Designing multi-step form field disclosure sequences
...
**Never:** Use both asterisks and "(optional)" in the same form — users must infer the convention from the pattern, and using both creates ambiguity about which convention takes precedence.

### Accessibility Requirements for Form Labels

Labels must be programmatically associated with their inputs or they are invisible to assistive technologies:
...
- Helper text should be associated via `aria-describedby` so screen readers read both the label and the helper text when the field is focused.
...
Helper text has spatial and cognitive limits by form context:

| Form Type        | Helper Text Limit | Example                                               |
| ---------------- | ----------------- | ----------------------------------------------------- |
| Simple inline    | 60 characters     | "Must be at least 8 characters"                       |
| Standard form    | 100 characters    | "Use the email associated with your Stripe account"   |
| Complex / wizard | 150 characters    | "This name will appear on invoices sent to customers" |
| Settings form    | 200 characters    | "Changing this affects all team members immediately"  |

When helper text exceeds the limit, consider whether the information belongs in a tooltip, a "Learn more" link, or inline documentation. Helper text that wraps to 4+ lines is probably documentation, not a hint.

### Anti-Patterns

1. **The Placeholder-as-Label.** Using placeholder text as the only label for a form field — no visible label above or beside the input. The placeholder disappears when the user clicks into the field, leaving them without context about what they were typing. Users who need to reference the field context while entering data (common for addresses, card numbers, and multi-field names) must clear their input to see the label again. This is a critical accessibility failure and consistently tested as one of the highest-friction patterns in form usability research. It is also a failure mode for users who tab between fields — the placeholder is gone before they begin typing.

2. **The Redundant Stack.** Three text elements saying the same thing: label "Name," placeholder "Enter your name," helper text "Type your name here." Each element adds visual weight and reading time with no added value. The fix: use each element for a distinct purpose. Label names the field ("Full name"), placeholder demonstrates format ("Jane Smith"), helper text adds non-obvious context ("As shown on your ID"). Redundancy signals that the form was built by someone who was uncertain about what each element should do — the solution is to understand their distinct roles.

3. **The Hidden Constraint.** Not revealing field requirements until after the user submits and receives a validation error. A password field with no helper text that, on error, reveals "Must be 8+ characters, include a number and uppercase letter." The user just had to fail to learn the rules. Move all constraints to helper text, visible before the user types. The only legitimate time to show constraints after the fact is when the constraint depends on the input value itself (e.g., "This username is already taken" cannot be preemptively shown because it depends on what the user typed).

4. **The Instruction Placeholder.** Placeholder text that instructs ("Enter your email") rather than formats ("jane@example.com") or demonstrates ("ACME-2024"). Instruction placeholders disappear on focus, leaving users who paused mid-entry without context. They also waste the placeholder's one legitimate use — demonstrating format. Instruction placeholders are almost always a sign that the label is doing insufficient work. The fix: strengthen the label to be descriptive, then use the placeholder for format demonstration only.

### Real-World Examples

**Stripe's Payment Form.** Stripe's payment form is a reference implementation of minimal, functional form labels. "Card number" is the label — not "16-digit credit or debit card number." The placeholder shows the format: "1234 1234 1234 1234." No helper text is needed because the label and placeholder together provide complete context. The expiry field placeholder "MM / YY" is the format instruction. The CVV placeholder "CVC" is the conventional abbreviation users recognize. Every text element earns its place; nothing is redundant. Stripe's checkout form has been through extensive usability testing, and the minimal labels are the result — earlier versions had more text, which was progressively removed as testing confirmed users did not need it.
...
**Shopify's Address Form Patterns.** Shopify's address forms demonstrate best-in-class required/optional field handling. All required fields are marked with asterisks (\*) and a legend explains the convention once at the form start. The "Address line 2" field is explicitly marked "(optional)" — because it is the exception in an otherwise-required form. Shopify also dynamically adjusts form fields based on selected country (postal code format, state/province terminology), with label text that adapts: "ZIP code" for US addresses, "Postcode" for UK addresses, "Postal code" for Canada. This locale-adaptive labeling is invisible when correct and jarring when wrong — Shopify gets it right.

**Airbnb's Listing Setup Form.** Airbnb's property listing form handles a particularly complex scenario: dozens of fields across multiple sections, many optional, with constraints that vary by property type. Airbnb's approach: group fields into named sections with clear visual dividers ("Location," "Amenities," "House rules"), mark required fields with asterisks on a "required only" form where most fields are required, and provide contextual helper text that explains why the information is needed: "Guests will only see your neighborhood until they book" below the address field. The explanation of data use reduces user hesitation about privacy-sensitive fields.
...
- Wroblewski, L. — _Web Form Design_ (2008), label placement research and required/optional field conventions
...
4. Audit required/optional indication — apply the minority-marking convention consistently across the form.
5. Check programmatic label association (for="" or aria-describedby) for accessibility compliance.

### Form Label Copy Review Checklist

Before shipping any form, verify each item:

| Check                                              | Pass Criteria                                       |
| -------------------------------------------------- | --------------------------------------------------- |
| All fields have persistent visible labels          | No placeholder-as-label in any field                |
| Labels use shortest unambiguous form               | "Email" not "Email address" when context is clear   |
| Placeholders show format only                      | "jane@example.com" not "Enter your email"           |
| Helper text appears below fields                   | Not above, not inside the input                     |
| Helper text adds unique value                      | Does not restate the label or placeholder           |
| Constraints shown before submission                | Password rules visible before the user types        |
| Required/optional marking uses minority convention | Only the minority type is marked                    |
| Forms over 6-8 fields have section headers         | Named groups, not a flat 15-field list              |
| Programmatic label association present             | for/id or aria-describedby on all inputs            |
| Locale-sensitive fields adapt labels               | "ZIP code" vs "Postcode" by country when applicable |

Form label audits consistently find placeholder-as-label as the most common violation. Running a quick audit by tabbing through a form with a screen reader reveals all label-association failures — any field that does not announce its label when focused has failed the accessibility check.
...
- Helper text that reveals constraints appears before the user types, not only after an error.
