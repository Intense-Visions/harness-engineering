...
2. **Auto-triggered retrospection at session archive** — the only _automated_
   ...
   `packages/cli/src/mcp/tools/state.ts` `handleArchiveSession`, gated at the
   `envEnabled(process.env.HARNESS_SESSION_RETROSPECTION)` check). It is gated behind
   **two** preconditions, both of which must hold:
   - env flag `HARNESS_SESSION_RETROSPECTION` is truthy (`envEnabled`: `1|true|yes|on`,
     case-insensitive) — **not set anywhere in this repo** (only referenced in a
     changeset doc);
...
- Emitter enablement is derived from env only, using the **same predicates the
  ...
  reuses `envEnabled` (`1|true|yes|on`, case-insensitive) and provider-resolvability
  mirrors `resolveAnalysisProvider`'s precedence (`ANTHROPIC_API_KEY` present, else
  `HARNESS_ANALYSIS_BASE_URL` present). Provider-resolvability is checked by env-var
  presence (not by constructing a provider) to keep `status` free of the
...
- **Registrations Required:** subcommand registered in `createProposalsCommand()`; the
  `envEnabled` predicate made reusable (export or shared-module hoist per D6); generated
  CLI reference (`docs/reference/cli-commands.md`) regenerated via `generate-docs`. No
  plugin/slash-command change (`proposals` is a CLI command, not a slash command — plugin
...
4. Unit tests cover `runProposalsStatus` across the enablement matrix (flag×provider)
   and queue tallying, and the reused `envEnabled` predicate. _(observable: test run)_
...
3. **Phase 3 — Verify + review.** Full build, `harness validate`, tests; confirm
   baseline byte-identical and plugin command count unchanged; run harness code review;
   address findings; open PR.
