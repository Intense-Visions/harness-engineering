...
- **Phase 4b — split-routing (D6/SC4).** **LANDED** (2026-07-11). The workflow stage-execution engine now exists and reads per-stage routing: a `>= 2`-stage workflow declared in `agent.workflows` is dispatched staged (behind the doubly-opt-in gate — declared workflow + `routing.policy` set), and each stage calls `route()` at its own required tier with the shared issue as the coherence unit. See `docs/changes/split-routing/proposal.md` and its four phase plans (`docs/changes/split-routing/plans/`): Phase 1 (engine + fake context single-exit), Phase 2 (per-stage routing + deterministic hints), Phase 3 (real stage failures — D8/D10), Phase 4 (`docs/changes/split-routing/plans/2026-07-11-phase-4-optin-producer-wiring-plan.md` — the opt-in gate + real declarative producer + live `dispatchIssue` wiring). The engine composes the **real** `WorkflowEngineContext` (`buildWorkflowContext`) from AMR + the backend factory + the recorder + the workspace manager; when no `>= 2`-stage workflow is declared, `dispatchIssue` is byte-behaviorally identical to single-agent dispatch (SC4). ADRs 0065/0066/0067/0068.
