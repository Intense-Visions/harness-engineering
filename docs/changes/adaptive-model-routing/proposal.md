...
- **Phase 4c — live quality-gate fan-in (D10/SC16).** The vertical escalation _mechanism_ is complete and unit-tested: `EscalationState` climbs a coherence unit's floor tier on the Nth consecutive quality failure (monotonic, `strong`-capped, `exhausted`-signalling); `AdaptiveRouter.recordOutcome` / the orchestrator's `recordAmrOutcome` seam feed it; and `onExhausted` now **hard-fails to a human** (queues a `needs-human` `routing:escalation-exhausted` interaction, not just a log). What is still NOT wired is a **live call site that emits `quality-fail`**: no per-`coherenceUnit` QUALITY verdict currently reaches `recordOutcome`. Investigation (2026-07-11) confirmed the orchestrator has **no clean quality signal available today**: worker exit `reason` (`normal`/`error`) is derived purely from whether the agent runner generator completed vs threw (`runAgentInBackgroundTask`), and the `ExecutionOutcome.result` recorded by `CompletionHandler` is derived from that _same_ runner-exit signal — so it is not an independent quality verdict. `PRDetector.branchHasPullRequest` returns only `{ found }` — it carries no PR-review decision, CI conclusion, or merged/blocked verdict back to the orchestrator, and there is no in-orchestrator gate/review/verify runner producing a per-unit pass/fail. A bare normal runner exit is therefore deliberately escalation-_neutral_ (it records nothing). **Wiring this soundly needs new plumbing across the review/CI feedback path** — e.g. a PR-review-decision / CI-conclusion sweep keyed to the coherence unit (issue), or an in-orchestrator quality-gate runner — that maps a _real_ gate/review failure to `recordOutcome(unit, lastRoutedTier, 'quality-fail')` (and a real pass to `'quality-pass'`), keeping transport failures out (breaker's job). It hooks at `emitWorkerExit`'s `outcomeClass` argument (already plumbed through `recordAmrOutcome`), which is why no orchestrator refactor is needed — only the missing upstream verdict source. Forcing the existing runner-exit signal into `recordOutcome` now would be unsound: it would record every clean agent exit as `quality-pass` (masking accumulating failures) and every runner crash as `quality-fail` (double-counting the transport breaker's job). **Net: SC16 is mechanism-satisfied + hard-fail-to-human-satisfied, not yet live-satisfied (no quality-fail emitter).** A 2026-07-12 re-investigation recorded the deferral decision in **ADR 0069** (`docs/knowledge/decisions/0069-amr-single-agent-quality-gate-deferred.md`) — and the sound `baseline-relative diff review` option it named was then **built (2026-07-13): single-agent SC16 is now live-satisfied.** On a normal single-agent exit the orchestrator scans only the ADDED lines of the agent's diff (working-tree vs `merge-base`, seeded overlay excluded) and feeds `quality-fail` on a new error-severity security finding — sound because every security rule is single-line, so pre-existing patterns never count (`quality-verdict.ts`, `WorkspaceManager.getIntroducedDiff`, `Orchestrator.deriveSingleAgentQualityVerdict`). The STAGED-workflow path was already live via the `pass-required` gate outcome. Still deferred: an LLM acceptance-eval verdict for spec-satisfaction (v1 covers security defects only).
- **Phase 4b — split-routing (D6/SC4).** **LANDED** (2026-07-11). The workflow stage-execution engine now exists and reads per-stage routing: a `>= 2`-stage workflow declared in `agent.workflows` is dispatched staged. Staged _execution_ is **single opt-in** — the workflow declaration alone selects the staged path (the `workflowFor` predicate reads only `config.workflows`); `routing.policy` is **not** required to run staged. Per-stage _tier routing_ is the additional opt-in: with a policy set each stage calls `route()` at its own required tier with the shared issue as the coherence unit; absent a policy each stage uses the identity/default chain (`adaptiveRouter: null` fallback). See `docs/changes/split-routing/proposal.md` and its four phase plans (`docs/changes/split-routing/plans/`): Phase 1 (engine + fake context single-exit), Phase 2 (per-stage routing + deterministic hints), Phase 3 (real stage failures — D8/D10), Phase 4 (`docs/changes/split-routing/plans/2026-07-11-phase-4-optin-producer-wiring-plan.md` — the opt-in gate + real declarative producer + live `dispatchIssue` wiring). The engine composes the **real** `WorkflowEngineContext` (`buildWorkflowContext`) from AMR + the backend factory + the recorder + the workspace manager; when no `>= 2`-stage workflow is declared, `dispatchIssue` is byte-behaviorally identical to single-agent dispatch (SC4). ADRs 0065/0066/0067/0068.
- **Phase 5 — orchestrator routing endpoints (harness side).** **LANDED** (2026-07-12, `docs/changes/amr-routing-endpoints/proposal.md`). The harness side of the Shuttle seam now exists: `PUT /api/v1/routing/policy` ingests a pushed `RoutingPolicy` and hot-swaps the live `AdaptiveRouter` (`ingestRoutingPolicy` — preserving `EscalationState`); `GET /api/v1/routing/telemetry` projects the enriched decision ring into Shuttle's wire shape (`{ decisions, spentUsd }`), fixing the cross-repo `RoutingDecision` mismatch; and `RoutingPolicy.allowedProviders` + the previously-dormant `selectCheapestQualifying` allowlist branch are now wired. Default-off is preserved (no policy pushed ⇒ `adaptiveRouter` stays `null`).
- **Phases 5–6 — Shuttle (cross-repo).** The Shuttle-side control plane (per-tenant `tenant-ai-config.routingPolicy`, the `settings/ai` editor, `ai_config_and_usage` cost attribution) and the autonomy hook + Meridian audit trail (`deriveAutonomyEligibility` consuming `EscalationState.isEscalated`, graduated Decide behind per-tenant `AuthorityScope`) live in the Shuttle SaaS repo. With the harness endpoints above landed, Shuttle's push/drain now has a real orchestrator to talk to (was mock-only).
