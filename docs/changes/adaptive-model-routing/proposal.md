...
- **Live classifier wiring.** Production AMR currently passes a constant `{ moderate, low }` complexity stub to `AdaptiveRouter` (the orchestrator construction seam), so enabling `routing.policy` yields provider-neutral cost routing at a **fixed** tier (standard, bumped to strong under the SC6 low-confidence rule) — NOT yet per-task difficulty-aware routing. Wiring the real `classify` cascade + live static-signal gathering (blast radius / diff / hotspot) into dispatch is deferred (pairs with the 4c fan-in). **SC1/SC2 are unit-satisfied via injected `req.complexity`, not yet live-satisfied.**
- **`PrivacyNoMatch` steward signal at the dispatch boundary.** The selector fails closed (correct: it never routes to a non-compliant backend), but at the dispatch site the throw is currently caught by the generic dispatch-error handler and classified `transport`, so the distinct `routing:no-tier-match` / steward-escalation observability promised in "Failure modes" (S4-001) is not yet wired. Low-impact today (`privacyFloor` is the only live exclusion; `allowedProviders` is Phase 5). Wire a `PrivacyNoMatch instanceof` branch when Phase 5 lands the allowlist.
- **Unify `RoutingError`.** The exported `RoutingError` (codes `privacy-no-match` / `escalation-exhausted`) is currently unused; the two codes are carried divergently (orchestrator-local `PrivacyNoMatch` + a `logger.warn('routing:escalation-exhausted')` string). Unify onto `RoutingError` (or drop it from the public API) when the 4c fan-in wires the real escalation-exhausted throw.
