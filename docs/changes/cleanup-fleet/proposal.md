...
4. **VERIFY.** For each returned branch, independently confirm — never by subagent self-report — that the cleanup **converged** (opening findings resolved, target re-scan clean) and that CI is green on all three OS plus the enforce and harness checks. A branch whose re-scan still shows findings did not converge and is rejected or retried.
