...
1. `### Phase 4: VERIFY — Three Independent Proofs, Never Self-Report`, numbered:
   ...
   7. **Plus behavior preservation and CI.** The passing-test count does not decrease; no public-API or observable-contract change; CI green on **all target operating systems** plus the project's required checks. Green on one OS is not green.
