...
1. **Print the full changed-file set against the base ref.**
   ...
   The output must be **exactly** these 12 paths (order irrelevant), and nothing else:
   ...
   (`proposal.md` and this plan file are the spec/plan artifacts already committed on this branch; both
   are in-scope lane artifacts.)
...
3. **Assert the diff is outside the edit set nowhere — mechanical set difference.**
   ...
   packages/orchestrator/tests/core/retry.test.ts
   EOF
     git diff --name-only fef03ac5ea510de1b109a76536c68c92a17f6771..HEAD | sort > /tmp/t9-actual.txt; \
