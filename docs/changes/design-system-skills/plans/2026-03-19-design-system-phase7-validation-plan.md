# Plan: Design System Phase 7 — Validation
...
Validate that all 15 success criteria from the design-system-skills spec are met by the artifacts produced in Phases 1-6, confirming that the full design skill family is complete and correct.

## Observable Truths (Acceptance Criteria)

1. A validation test file exists at `packages/cli/tests/design-system/validation.test.ts` that programmatically checks all 15 success criteria.
...
### Task 1: Create the validation test file — criteria 1-9 (skill content checks)

**Depends on:** none
**Files:** packages/cli/tests/design-system/validation.test.ts

1. Create directory `packages/cli/tests/design-system/` if it does not exist.
2. Create test file `packages/cli/tests/design-system/validation.test.ts`:
...
describe('Design System Skills — Phase 7 Validation', () => {
  ...
  // (This is verified by running `pnpm test` — covered by Task 3)
...
3. Run test: `npx vitest run packages/cli/tests/design-system/validation.test.ts`
4. Observe: all tests pass
5. Run: `harness validate`
6. Commit: `test(design-system): add Phase 7 validation test covering all 15 success criteria`

### Task 2: Run full monorepo test suite

**Depends on:** Task 1
**Files:** none (verification only)

1. Run: `pnpm test`
2. Observe: all packages pass. Record the test counts:
   - `@harness-engineering/cli`: X tests
   - `@harness-engineering/graph`: X tests
   - `@harness-engineering/mcp-server`: X tests
3. Run: `harness validate`
4. Record results for the verification report.

### Task 3: Run graph package tests specifically

**Depends on:** none (can parallel with Task 1)
**Files:** none (verification only)

1. Run: `npx vitest run packages/graph/tests/ingest/DesignIngestor.test.ts`
2. Observe: all DesignIngestor tests pass (12 tests covering token ingestion, design intent ingestion, ingestAll)
3. Run: `npx vitest run packages/graph/tests/constraints/DesignConstraintAdapter.test.ts`
4. Observe: all DesignConstraintAdapter tests pass (8 tests covering hardcoded colors, hardcoded fonts, severity mapping, checkAll)
5. Run: `npx vitest run packages/cli/tests/config/design-schema.test.ts`
6. Observe: all DesignConfigSchema tests pass (16 tests covering strictness, platforms, tokenPath, aestheticIntent)

### Task 4: Validate shared data files load as valid YAML

**Depends on:** none (can parallel with Task 1)
**Files:** none (verification only — covered by Task 1 tests)

1. Verify all 18 YAML files under `agents/skills/shared/design-knowledge/` parse without errors:
   - `industries/`: 8 files (saas, fintech, healthcare, ecommerce, creative, emerging-tech, lifestyle, services)
   - `anti-patterns/`: 4 files (typography, color, layout, motion)
   - `palettes/`: 1 file (curated)
   - `typography/`: 1 file (pairings)
   - `platform-rules/`: 4 files (web, ios, android, flutter)
2. This is programmatically verified by SC10 in the validation test (Task 1).
...
1. Create `docs/plans/2026-03-19-design-system-phase7-VERIFICATION.md` documenting:
   ...
   - Any issues found
2. Update `.harness/handoff.json` with Phase 7 completion.
3. Run: `harness validate`
4. Commit: `docs(design-system): add Phase 7 validation verification report`
...
- **Task commands** — Every task includes exact harness CLI commands to run.

## Notes

- The `yaml` package is already a devDependency in the CLI package (used by skill normalization).
- SC11 (all tests pass) is covered by running `pnpm test` in Task 2.
- SC13 (graph ingestion) is covered by existing DesignIngestor.test.ts + DesignConstraintAdapter.test.ts, both with passing tests.
- The validation test reads skill files from disk using fs, not import — this is the correct approach for validating markdown/YAML content.
- From learnings: Prettier may reformat JSON in SKILL.md during pre-commit. The test compares content between platforms using exact string equality, which works because both copies are formatted identically by Prettier (per Phase 3 learning).
- From learnings: persona count tests are fragile. This plan does NOT add any new personas, so no count updates needed.
