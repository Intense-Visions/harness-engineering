...
## Addendum (2026-05-01) — Post-Phase-1 widening

`localModel?: string` was widened to `localModel?: string | string[]` to support a prefer-and-fallback list of candidate model IDs. The orchestrator now constructs a `LocalModelResolver` (see `packages/orchestrator/src/agent/local-model-resolver.ts`) that probes `/v1/models` periodically (default 30s) and selects the first configured candidate present in the server's response. When no candidate is loaded, locally-routed paths are disabled with a typed `agent_not_found` error and a dashboard banner; cloud-routed paths are unaffected. The intelligence pipeline does not auto-rebuild on availability flips — restart the orchestrator after loading a model to re-enable spec enrichment / pre-execution simulation.

See `docs/changes/local-model-fallback/proposal.md` for the full spec, and ADRs [0003](../../knowledge/decisions/0003-local-model-resolution-strategy.md) and [0004](../../knowledge/decisions/0004-local-availability-disables-not-escalates.md) for the resolution strategy and disable-not-escalate decisions.
