...
4. **VERIFY.** For each PR proposed to land, independently confirm — never by subagent self-report — that CI is green on all three OS plus the project's required checks, that a review verdict exists, and that the PR is mergeable (no conflicts, base current). A PR missing any of these is not landable and is reported, not landed.
