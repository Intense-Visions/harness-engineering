...
4. **VERIFY.** For each returned branch, independently confirm — never by sub-agent self-report — that the plan artifact (`docs/changes/<slug>/plans/`) and an autopilot-state exist, and that CI is green on all three OS plus the project's required checks. An item lacking a plan artifact did not run the pipeline and is rejected or retried.
