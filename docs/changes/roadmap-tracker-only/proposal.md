...
| D7  | Coordination via thin client + per-process ETag store + `If-None-Match` reads + `If-Match` writes (412 retry)  | Solves the rate-limit problem with a well-trodden HTTP pattern; no broker, no central cache, no new process; keeps architecture flat                                                                             |
...
| F2  | Two concurrent claim attempts on the same feature produce one success and one `ConflictError`                                                             | Integration test: two clients race `claim()` on the same external ID with stale ETags                              |
