...
3. **The evidence gate is a hard filter with four named evidence classes.** A candidate enters the batch only if it carries at least one of:
   ...
   - **`advisory-match`** — a resolved dependency version falls inside an advisory's affected range. The range match _is_ the class: a resolved version inside a published affected range is a concrete fact, not a guess. Reachability is a **strength modifier, not a second gate** — `evidenceStrength` is `reached` (the vulnerable API surface is actually called here) or `lockfile-only` (present in the resolved tree, no call path found). Strength feeds the score and the tier; a `lockfile-only` match ranks below a `reached` one and is never inflated.
   - **`exposed-credential`** — a live-shaped secret literal is present in tracked source, evidenced by **file, line, and credential type**; the value itself is never recorded or transported. A placeholder, an obvious test fixture, or a documented example value is not a live-shaped secret. Its evidence-cleared branch in VERIFY is the literal being gone from tracked source **and** rotation recorded as still outstanding — removing a credential from source does not un-leak it.
...
- **Every item in the batch carries one of the four named evidence classes**; candidates without one are discarded before CONFIRM and reported only as an aggregate count.
