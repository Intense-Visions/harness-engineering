...
> **Scope note (discovered during execution):** the strengthened structure test flagged **24** behavioral skills missing the section, not 19. Five more (`api-craft`, `cli-ergonomics-craft`, `code-craft`, `docs-craft`, `harness-rehearse`) drifted in after this work was scoped — which is precisely the drift class this change closes. All 24 are authored so the two gates reach zero-gap agreement; leaving any unauthored would keep the strengthened gate red. The lists of 19 below are the originally-scoped set; the delivered set is those 19 plus those 5.

## Decisions Made

- **D1 — Single source of truth in `@harness-engineering/core`, not a parity assertion.** Both `packages/cli` and `agents/skills` already depend on `@harness-engineering/core` (`workspace:*`); neither the reverse (`agents/skills` does not depend on `cli`). A shared constant exported from core and imported by both gates makes drift _structurally impossible_ — strictly stronger than a test that only _detects_ drift after someone re-inlines a list. This aligns with the STRATEGY track "constraints-as-code outperforms prompts-and-conventions": encode the rule once, mechanically, rather than maintaining two conventions that must be kept in sync by discipline.
