...
### Pattern 4: `needs-spec` Label → Brainstorming Agent

When an issue is labeled `needs-spec`, a headless agent runs the brainstorming skill and posts a spec draft as a comment.
    ...
    steps:
      ...
      - name: Generate spec draft
        env:
          ...
          GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
        run: |
          # Extract issue body as context
          gh issue view ${{ github.event.issue.number }} --json body -q '.body' > context.md

          # Run brainstorming skill headlessly
          harness agent run \
            --headless \
          ...
          gh issue comment ${{ github.event.issue.number }} \
            --body "## Auto-Generated Spec Draft"$'\n\n'"$(cat spec-draft.md)"$'\n\n'"---"$'\n'"*Generated by harness headless agent*"
```

See the full [headless agent action recipe](./recipes/headless-agent-action.yml).

### Pattern 5: `needs-plan` Label → Planning Agent

Same pattern as above, but runs the planning skill against a linked spec.
