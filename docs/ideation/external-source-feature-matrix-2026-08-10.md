...
**Filed as #1285 and #1286.** Root cause for the two mutation symptoms was traced to a single origin: `shouldTriggerExternalSync` (`packages/cli/src/mcp/tools/roadmap.ts:757`) returns `true` for `add`, which fires `triggerExternalSync` → `fullSync` (`roadmap-auto-sync.ts:88`) — a bidirectional reconcile across **every** row as a side effect of inserting one. The 7 affected shards are precisely those whose local value disagreed with the tracker: three had a local assignee GitHub lacks, four were local `backlog` against a merely-`OPEN` issue. `groom` is already excluded from this path with a comment explaining why mirroring a local reorganization is wrong; the same reasoning applies to `add`. The truncated-shard defect is filed separately as #1286 because it is an incomplete write to the correct new shard rather than a stray write to the wrong ones.
