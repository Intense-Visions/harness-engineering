# Plan: harness-i18n-workflow Skill (Phase 3)
...
Create the `harness-i18n-workflow` skill (skill.yaml + SKILL.md) for both claude-code and gemini-cli platforms, enabling AI agents to manage the translation lifecycle -- configuration, scaffolding, string extraction, coverage tracking, pseudo-localization, and retrofit of existing projects.

## Observable Truths (Acceptance Criteria)

1. The file `agents/skills/claude-code/harness-i18n-workflow/skill.yaml` exists with `cognitive_mode: constructive-architect`, `type: flexible`, triggers `[manual, on_project_init]`, phases `[configure, scaffold, extract, track]`, and `depends_on: [harness-i18n]`.
2. The file `agents/skills/claude-code/harness-i18n-workflow/SKILL.md` exists with sections: When to Use, Process (4 phases: CONFIGURE, SCAFFOLD, EXTRACT, TRACK), Harness Integration, Success Criteria, Examples, Gates, Escalation.
3. The file `agents/skills/gemini-cli/harness-i18n-workflow/skill.yaml` is byte-identical to the claude-code copy.
4. The file `agents/skills/gemini-cli/harness-i18n-workflow/SKILL.md` is byte-identical to the claude-code copy.
5. `harness validate` passes after all changes.
6. When the SKILL.md CONFIGURE phase instructions are followed, the agent sets up `i18n` config in `harness.config.json` with opinionated defaults (ICU MessageFormat, JSON format, dot-notation keys, `en` source locale, `locales/{locale}/{namespace}.json` structure).
7. When the SKILL.md SCAFFOLD phase instructions are followed, the agent creates translation file directories and locale files based on config, and in retrofit mode audits existing code via `harness-i18n` to generate an initial key catalog.
8. When the SKILL.md EXTRACT phase instructions are followed, the agent extracts translatable strings into translation files, generates keys following the configured naming convention, preserves existing translations, and flags strings needing human review.
9. When the SKILL.md TRACK phase instructions are followed, the agent reports per-locale coverage percentages, untranslated values, stale translations, missing plural forms, generates pseudo-localization, and recommends MCP servers based on workflow needs.

## File Map

- CREATE `agents/skills/claude-code/harness-i18n-workflow/skill.yaml`
- CREATE `agents/skills/claude-code/harness-i18n-workflow/SKILL.md`
- CREATE `agents/skills/gemini-cli/harness-i18n-workflow/skill.yaml` (copy)
- CREATE `agents/skills/gemini-cli/harness-i18n-workflow/SKILL.md` (copy)

## Tasks

### Task 1: Create skill.yaml for harness-i18n-workflow

**Depends on:** none
**Files:** `agents/skills/claude-code/harness-i18n-workflow/skill.yaml`

1. Create directory `agents/skills/claude-code/harness-i18n-workflow/`.
2. Create `agents/skills/claude-code/harness-i18n-workflow/skill.yaml` with the following exact content:
...
4. Commit: `feat(i18n-workflow): add skill.yaml for harness-i18n-workflow`

### Task 2: Create SKILL.md for harness-i18n-workflow

**Depends on:** Task 1
**Files:** `agents/skills/claude-code/harness-i18n-workflow/SKILL.md`

1. Create `agents/skills/claude-code/harness-i18n-workflow/SKILL.md` with the following exact content:
...
3. Commit: `feat(i18n-workflow): add SKILL.md for harness-i18n-workflow`

### Task 3: Copy skill files to gemini-cli platform

**Depends on:** Task 2
**Files:** `agents/skills/gemini-cli/harness-i18n-workflow/skill.yaml`, `agents/skills/gemini-cli/harness-i18n-workflow/SKILL.md`

1. Create directory `agents/skills/gemini-cli/harness-i18n-workflow/`.
2. Copy `agents/skills/claude-code/harness-i18n-workflow/skill.yaml` to `agents/skills/gemini-cli/harness-i18n-workflow/skill.yaml`.
3. Copy `agents/skills/claude-code/harness-i18n-workflow/SKILL.md` to `agents/skills/gemini-cli/harness-i18n-workflow/SKILL.md`.
4. Verify byte-identical: `diff agents/skills/claude-code/harness-i18n-workflow/skill.yaml agents/skills/gemini-cli/harness-i18n-workflow/skill.yaml` (should produce no output).
5. Verify byte-identical: `diff agents/skills/claude-code/harness-i18n-workflow/SKILL.md agents/skills/gemini-cli/harness-i18n-workflow/SKILL.md` (should produce no output).
6. Run: `harness validate`
7. Commit: `feat(i18n-workflow): add gemini-cli platform copy of harness-i18n-workflow`

### Task 4: Run tests and final validation

**Depends on:** Task 3
**Files:** none (validation only)

1. Run: `npx vitest run` to confirm no regressions (expect ~558+ tests passing, including the 20 i18n schema tests from Phase 2).
2. Run: `harness validate` -- must pass.
3. Verify the new skill appears in the skill registry by checking that `harness validate` counts the new skill's automatic tests.
4. Run: `diff agents/skills/claude-code/harness-i18n-workflow/skill.yaml agents/skills/gemini-cli/harness-i18n-workflow/skill.yaml` -- no output.
5. Run: `diff agents/skills/claude-code/harness-i18n-workflow/SKILL.md agents/skills/gemini-cli/harness-i18n-workflow/SKILL.md` -- no output.
...
| 3. gemini-cli skill.yaml byte-identical                     | Task 3                    |
| 4. gemini-cli SKILL.md byte-identical                       | Task 3                    |
| 5. harness validate passes                                  | Task 4                    |
| 6. CONFIGURE phase has opinionated defaults                 | Task 2 (SKILL.md Phase 1) |
| 7. SCAFFOLD phase creates files and handles retrofit        | Task 2 (SKILL.md Phase 2) |
| 8. EXTRACT phase generates keys and preserves translations  | Task 2 (SKILL.md Phase 3) |
| 9. TRACK phase reports coverage and generates pseudo-locale | Task 2 (SKILL.md Phase 4) |
