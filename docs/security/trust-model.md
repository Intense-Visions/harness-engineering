...
1. A maintainer publishes a new `@harness-engineering/cli` version (changesets → npm,
   with provenance).
2. The pin in the plugin manifests is bumped to that version **automatically** — the
   root `version` script runs `scripts/sync-plugin-pin.mjs` right after
   `changeset version`, so the changesets "Version Packages" PR carries the manifest
   pin bump in the same commit that bumps `packages/cli/package.json` (see
...
The pin is **auto-synced**, not hand-maintained. `scripts/generate-plugin.mjs` still
does not regenerate the manifests (that generator only produces commands, agents, and
`hooks.json`), and `changesets` on its own does not touch them — but the root `version`
...
When a release runs (see `.github/workflows/release.yml`), `changeset version` bumps
`packages/cli/package.json`, then `scripts/sync-plugin-pin.mjs` reads that version and
rewrites the `@harness-engineering/cli@<version>` token in the four manifests to match:
...
So the manifest pin bump lands in the **same commit** the changesets "Version Packages"
PR uses to bump the CLI version — the pin ships in lockstep with the release it is
published from. The script is idempotent (a no-op when the pins already match) and
touches only the version number, so it produces no formatting churn.

This does **not** remove the human review gate — it relocates it. A maintainer still
reviews and merges the "Version Packages" PR before the release publishes, so the pin
moves **deliberately, not silently**; the automation only removes the manual, forgettable
step of hand-editing four files. A drift guard
(`tests/scripts/plugin-pin-sync.test.mjs`) asserts the manifest pins equal
`packages/cli/package.json`, so a hand-edit or a skipped sync is caught in review.

The `README.md` _Updates_ section uses a generic `<pinned-version>` placeholder rather
than an exact version token, so it is intentionally **not** rewritten by the script and
needs no per-release edit. To read the current published latest:

```bash
npm view @harness-engineering/cli version
```

Syncing to the released version automatically (rather than tracking `@latest` at runtime)
preserves the whole point of the pin: the review of the "Version Packages" PR is the gate
between a new publish and every adopter.
