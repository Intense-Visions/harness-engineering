...
export async function runAdd(
  ...
  try {
    await addComponent(componentType, name, cwd, configResult, created);
    return Ok({ created });
  } catch (error) {
    return Err(
      new CLIError(
...
async function addLayer(name: string, cwd: string, created: string[]): Promise<void> {
...
function addModule(name: string, cwd: string, created: string[]): Result<void, CLIError> {
...
function addDoc(
  name: string,
  cwd: string,
  configResult: ReturnType<typeof resolveConfig>,
  created: string[]
...
async function addSkill(name: string, cwd: string, created: string[]): Promise<void> {
...
function addPersona(name: string, cwd: string, created: string[]): Result<void, CLIError> {
...
async function addComponent(
  ...
  switch (componentType) {
    case 'layer':
      await addLayer(name, cwd, created);
      return;
    case 'module': {
      const r = addModule(name, cwd, created);
      if (!r.ok) throw r.error;
      return;
    }
    case 'doc': {
      const r = addDoc(name, cwd, configResult, created);
      if (!r.ok) throw r.error;
      return;
    }
    case 'skill':
      await addSkill(name, cwd, created);
      return;
    case 'persona': {
      const r = addPersona(name, cwd, created);
      if (!r.ok) throw r.error;
      return;
    }
    default: {
