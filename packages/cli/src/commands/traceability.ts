...
function handleNoStore(mode: OutputModeType): never {
  if (mode === OutputMode.JSON) {
    console.log(JSON.stringify({ error: 'No knowledge graph found. Run `harness graph scan` first.' }));
  } else {
...
function handleEmptyResults(mode: OutputModeType): never {
  if (mode === OutputMode.JSON) {
    console.log(
      JSON.stringify({
        results: [],
        message: 'No requirements found in graph. Run `harness graph scan` to ingest spec requirements.',
      })
    );
  } else if (mode !== OutputMode.QUIET) {
    logger.info('No requirements found in graph. Run `harness graph scan` to ingest spec requirements.');
  }
