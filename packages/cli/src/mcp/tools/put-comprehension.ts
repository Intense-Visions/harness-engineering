...
export interface AttachSemanticDeps {
  ...
  /**
   * ADR 0116 §1 — single-writer policy gate: may THIS session write committed
...
export type PutComprehensionOutcome =
  ...
  // ADR 0116 §1 — the PR path is static-only: committed semantic is written only
...
export async function attachSemantic(
  ...
  // ADR 0116 §1 — single writer: committed semantic is written only on the `main`
  ...
  if (deps.committedSemanticAllowed && !deps.committedSemanticAllowed()) {
    return {
      ...
      module,
      reason:
        'single-writer policy (ADR 0116): committed semantic is written only on the `main` ' +
...
function resolveDefaultDeps(projectRoot: string): AttachSemanticDeps {
  return {
    ...
    // ADR 0116 §1 — enforce single-writer in production: on a branch this refuses
...
export async function handlePutComprehension(
  ...
  try {
    ...
    // ADR 0116 single-writer deferral from a stale/missing unit.
    return textEnvelope(
      JSON.stringify({
        module: outcome.module,
        written: false,
