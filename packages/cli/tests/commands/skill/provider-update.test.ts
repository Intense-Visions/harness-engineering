...
describe('updateProviders', () => {
  ...
  it.each([
    ...
    'skips a github provider whose %s carries an embedded %s delimiter (spec injection)',
    async (field, value) => {
      const bad = {
        ...gh,
        source: {
  ...
  it('allows a slash-containing branch ref (round-trips cleanly, not unsafe)', async () => {
    const ok = {
      ...gh,
      source: { kind: 'github', owner: 'o', repo: 'r', ref: 'feature/foo', commit: 'old' },
    } as ProbedProvider;
    ...
    expect(mockedInstall).toHaveBeenCalledWith(
      '@harness-skills/gh',
      expect.objectContaining({ from: 'github:o/r#feature/foo' })
    );
  });
  ...
  it('still allows a plain HEAD-relative ref with no delimiters', async () => {
    const ok = {
      ...gh,
      source: { kind: 'github', owner: 'o', repo: 'r', ref: 'v1.2.3', commit: 'old' },
    } as ProbedProvider;
    await updateProviders([ok], { yes: true });
    expect(mockedInstall).toHaveBeenCalledWith(
      '@harness-skills/gh',
      expect.objectContaining({ from: 'github:o/r#v1.2.3' })
    );
  });
