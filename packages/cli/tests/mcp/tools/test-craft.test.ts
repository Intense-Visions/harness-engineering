...
} from '../../../src/mcp/tools/test-craft';

const TEST_FILE = `import { describe, it, expect } from 'vitest';

describe('classify', () => {
  it('returns big for large input', () => {
    expect(classify(20)).toBe('big');
  });
});
`;

describe('test_craft MCP tool', () => {
  let tmpDir: string;
  let savedEnv: string | undefined;

  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'test-craft-mcp-'));
  ...
  it('definition exposes mode and promptBudget on input schema', () => {
    expect(testCraftDefinition.inputSchema.properties).toHaveProperty('mode');
    expect(testCraftDefinition.inputSchema.properties).toHaveProperty('promptBudget');
  });

  it('finalize definition declares path, runId, responses as required', () => {
    expect(testCraftFinalizeDefinition.inputSchema.required).toEqual([
      'path',
      'runId',
      'responses',
    ]);
  ...
  it('returns pendingPrompts when mode=in-session (not a silent empty result)', async () => {
    fs.writeFileSync(path.join(tmpDir, 'foo.test.ts'), TEST_FILE);
    const r = await handleTestCraft({ path: tmpDir, mode: 'in-session' });
  ...
  it('default (in-session) mode collects prompts rather than returning findings:[]', async () => {
    ...
    const r = await handleTestCraft({ path: tmpDir });
    expect(r.isError).toBeFalsy();
  ...
  it('routes a round-trip through test_craft → test_craft_finalize', async () => {
    fs.writeFileSync(path.join(tmpDir, 'foo.test.ts'), TEST_FILE);
    process.env.HARNESS_CRAFT_LLM = 'in-session';
    const collect = await handleTestCraft({ path: tmpDir });
    ...
    const finalize = await handleTestCraftFinalize({
      path: tmpDir,
  ...
  it('finalize errors when runId is unknown/missing', async () => {
    const r = await handleTestCraftFinalize({
      path: tmpDir,
      runId: 'does-not-exist',
      responses: [],
    });
    expect(r.isError).toBe(true);
    expect(r.content[0]!.text).toContain('no persisted run');
  });
