...
- a966085: Make the complexity detector's `findFunctionEnd` literal- and comment-aware. A
...
- d51e370: Fix roadmap serialize/parse round-trip silently truncating a multi-line
...
- d3673bf: fix(harness-strength): scale the strength score by audit coverage so partial
...
- f231e90: Fix `syncRoadmap` corrupting `planned` features with named `Blockers:` into `blocked` status.
