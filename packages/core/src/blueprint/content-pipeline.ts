...
export class ContentPipeline {
  async generateModuleContent(module: BlueprintModule): Promise<Content> {
    ...
    const translation = await llmService.generate(`You are a technical educator. Explain the following code clearly and concisely: ${codeContext}`);
    const quizJson = await llmService.generate(`Create 3 technical quiz questions for this code. Return ONLY valid JSON in this format: { "questions": [{ "question": "...", "answer": "..." }] }. Code: ${codeContext}`);
