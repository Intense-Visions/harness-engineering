 ...
 *
 * The concrete `ExtractStatic` (D1/D5): {@link compileModule} stays pure and
 * receives the extractor injected. Promoted into core (from the cli) so any
 * consumer — the `harness comprehend` CLI and library consumers alike — shares
 * one AST extractor over core's {@link TypeScriptParser}, rather than each
 * re-implementing the interface/dependency rendering.
 *
 * The render helpers below are pure and deterministic (dedup + sort), and never
 * fabricate a surface — an empty input yields an empty section so an
 * unsupported/parse-degraded module produces a static-degraded (semantic-only)
 * unit rather than a faked one.
 */
