...
function loadChecksFromConfig(gateConfigPath: string): Check[] {
  if (!fs.existsSync(gateConfigPath)) return [];
...
function discoverChecksFromProject(projectPath: string): Check[] {
  const checks: Check[] = [];
...
function executeCheck(check: Check, projectPath: string): GateResult['checks'][number] {
  ...
  try {
    ...
    const output =
      error instanceof Error
        ? (error as Error & { stderr?: { toString(): string } }).stderr?.toString() || error.message
        : String(error);
    return {
      name: check.name,
      passed: false,
