...
describe('detectCircularDepsInFiles', () => {
  ...
  it('should propagate error when buildDependencyGraph fails', async () => {
    const mockParser = {
      name: 'mock',
      extensions: ['.ts'],
      parseFile: async () => Err(createError('PARSE_ERROR', 'Failed to parse', {}, [])),
      extractImports: () => Err(createError('PARSE_ERROR', 'Failed', {}, [])),
      extractExports: () => Err(createError('PARSE_ERROR', 'Failed', {}, [])),
      health: async () => ({ ok: true, value: { available: true } }),
    } as unknown as LanguageParser;

    const result = await detectCircularDepsInFiles(['nonexistent.ts'], mockParser);

    // The function should still succeed even if files can't be parsed
    // because buildDependencyGraph skips unparseable files
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.hasCycles).toBe(false);
    }
...
describe('detectCircularDeps edge cases', () => {
  ...
  it('should handle edges to nodes not in the graph', () => {
    const graph: DependencyGraph = {
      nodes: ['a.ts', 'b.ts'],
      edges: [
        { from: 'a.ts', to: 'b.ts', importType: 'static', line: 1 },
        { from: 'b.ts', to: 'external.ts', importType: 'static', line: 1 }, // external.ts not in nodes
      ],
    };
